...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, defaultFilename=None, defaultPrefdefClassFile=None):
        ...
        multilineSelect = action('Multiline Select(F)', self.multilineSelect, 'f', 'multilineSelect', u'desc', enabled=True)
        ...
        self.actions = struct(save=save, saveAs=saveAs, open=open, close=close,
                              ...
                              create=create, createRo=createRo, toggleAutoResize=toggleAutoResize, resizeSelected=resizeSelected, multilineSelect=multilineSelect, delete=delete, edit=edit, copy=copy,
        ...
        self.actions.beginner = (
            open, opendir, openNextImg, openPrevImg, verify, save, None, create, createRo, toggleAutoResize, resizeSelected, multilineSelect, copy, delete, None,
    ...
    def setClean(self):
        ...
        self.actions.multilineSelect.setEnabled(True)
    ...
    def createShape(self):
        ...
        self.actions.multilineSelect.setEnabled(False)
    ...
    def newShape(self):
        ...
        """
        is_shift = QApplication.keyboardModifiers() == Qt.ShiftModifier
        ...
        if text is not None and self.isMultilineSelect:
            self.isMultilineSelect = False
            self.addLabel(self.canvas.setLastLabel(text))
            self.new_multiline_shape(is_shift)
            return
    ...
    def new_multiline_shape(self, vertical=False):
        max_bounds_shape = self.canvas.shapes.pop()
        self.remLabel(max_bounds_shape)
        x1, y1, x2, y2 = LabelFile.convertPoints2BndBox(((p.x(), p.y()) for p in max_bounds_shape.points))
        bg_color = self.image_np_array[y1, x1]
        prev_is_word_pixel = False
        min_coord = y1
        for i in range(x1 if vertical else y1, x2 + 1 if vertical else y2 + 1):
            slice = self.image_np_array[y1:y2 + 1, i] if vertical else self.image_np_array[i, x1:x2 + 1]
            is_word_pixel = contains_word_pixels(slice, bg_color)
            if is_word_pixel and not prev_is_word_pixel:
                min_coord = i
            elif not is_word_pixel and prev_is_word_pixel:
                if (i - min_coord) < 10:
                    prev_is_word_pixel = is_word_pixel
                    continue
                new_shape = max_bounds_shape.copy()
                if vertical:
                    new_shape.points[0].setX(min_coord)
                    new_shape.points[1].setX(i)
                    new_shape.points[2].setX(i)
                    new_shape.points[3].setX(min_coord)
                else:
                    new_shape.points[0].setY(min_coord)
                    new_shape.points[1].setY(min_coord)
                    new_shape.points[2].setY(i)
                    new_shape.points[3].setY(i)
                self.canvas.shapes.append(new_shape)
                self.addLabel(new_shape)
                self.resize_shape(new_shape)
            prev_is_word_pixel = is_word_pixel
        if self.beginner():  # Switch to edit mode.
            self.canvas.setEditing(True)
            self.actions.create.setEnabled(self.isEnableCreate)
    ...
    def multilineSelect(self):
        assert self.beginner()
        self.canvas.setEditing(False)
        self.canvas.canDrawRotatedRect = False
        self.isMultilineSelect = True
        self.actions.create.setEnabled(False)
        self.actions.createRo.setEnabled(False)
        self.actions.multilineSelect.setEnabled(False)
    ...
    def resize_shape(self, shape):
        ...
        for i in range(y1, y2 + 1):
            if contains_word_pixels(self.image_np_array[i, x1:x2+1], bg_color):
                y1 = i - MARGIN
                print('top', y1)
                break
            # bottom
        ...
        for i in range(x1, x2 + 1):
            if contains_word_pixels(self.image_np_array[y1:y2+1, i], bg_color):
                x1 = i - MARGIN
                print('left', x1)
                break
            # right
...
def contains_word_pixels(arr, bg_color):
    return 0 < np.average(np.abs(arr - bg_color) > THRESHOLD) < 0.92
